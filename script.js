...
ngetikh = 0;
function ngetikC() {
  if (ah < ngetikh.length) {
    kalimatc.innerHTML += ngetikh.charAt(ah);
    ah++;
    setTimeout(ngetikC, 60);
  }
  if (ah == ngetikh.length) {
    setTimeout(otomatisc2, 1000);
  }
}
...
function otomatisc2() {
  ...
  befanimkatac();
  setTimeout(animkatac, 400);
}
function befanimkatac() {
  kalimat.style.opacity = "0";
  kalimatb.style.opacity = "0";
  kalimatc.style.opacity = "0";
  skalimatc = kalimatc1.innerHTML;
  kalimatc.innerHTML = "";
}
function animkatac() {
  kalimat.innerHTML = kalimat1.innerHTML;
  kalimatb.innerHTML = kalimatb1.innerHTML;
  kalimatc.innerHTML = kalimatc1.innerHTML;
...
function otomatis3a() {
  setTimeout(sbakhir, 700);
  befanimkata3a();
  setTimeout(animkata3a, 400);
}
function befanimkata3a() {
  kalimatc.style.opacity = "0";
}
function animkata3a() {
...
function otomatis3b() {
  befanimkata3b();
  setTimeout(animkata3b, 400);
}
function befanimkata3b() {
  kalimat.style = "transform:scale(.3);";
}
function animkata3b() {
  kalimatc.innerHTML = kalimatc3b.innerHTML;
  kalimat.style = "transform:scale(1);";
}

function fangka() {
  angka.innerHTML = Math.floor(Math.random() * 25) + 5;
}
function henti() {
  clearInterval(tmer);
...
async function lanjut() {
  ...
  bq.style =
    "position:relative;opacity:1;visibility:visible;transform: scale(1);transition:all .7s ease;margin-top:0;";
...
async function ditolak() {
  if (fungsi == 1) {
    Tombol.style = "transition:all .3s ease;opacity:0";
    await swalst.fire({
...
function mulaict() {
  ...
  var downloadTimer = setInterval(function () {
    ...
    if (timeleft >= 6) clearInterval(downloadTimer);
    if (timeleft == 6) {
